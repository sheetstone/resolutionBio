var aspect = 300 / 300;
    ...
    var margin = {top: 0, right: 0, bottom: 0, left: 0},
       width = $("#MSKContainer").width() - margin.left - margin.right,
       // height = $("#MSKContainer").width() - margin.top - margin.bottom;
       //width = 900;
       height = 900;

var svg = d3.select("#MSKContainer").append("svg")
        .attr("width", width + margin.left + margin.right)
        .attr("height", height + margin.top + margin.bottom);

	$("#dataWindow").css("top",190).css("left",0);
...
var cluster = d3.cluster().size([360, 390]);

d3.csv("/js/msk.csv", function(error, data) {
	...
	var link = g.selectAll(".link")
		...
		.attr("stroke", function(d) { if(d.depth == 2) {
            var ID = d.children[0].data.id.substring(d.children[0].data.id.lastIndexOf(".") + 1).split(":"); return "url(#"+ID[0]+"Gradient)"; }
        })
		.attr("d", function(d) {
	...
	node.append("circle")
		...
		.attr("r",function(d) {
			...
			if(d.children) {
				...
				if(t == 'TP53') {
					return d.children.length* 0.6;
				} else {
					return d.children.length * 1.5;
				}
			} else {
				return 0;
			}
	...
	node.append("text")
        ...
        // mutation ring d.x is distannce of the text from the center
		.attr("x",function(d) {
			if(d.depth == 1) {
                // genes
				return d.x < 180 ? 230 : -230;
			} else {
                // mutations
				return d.x < 180 ? 9 : -9;
			}
		...
		.text(function(d) {
			...
			if(t.length > 1) {
                ...
                // mutation text
                if(d.children.length > 1 && d.depth == 2) {
                        return t[0].replace(".","*").replace("_",".")+' ('+d.children.length+')';
                } else {
